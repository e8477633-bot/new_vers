...
app.post('/api/lecturer/login', (req, res) => {
    ...
    if (username === `lecturer_${programCode}_l${level}` && password === 'lecturer123') {
        res.json({ 
            success: true, role: 'lecturer', 
            name: program?.name || programCode,
            fullname: `Lecturer - ${program?.name || programCode}`,
            programCode, programName: program?.name || programCode, 
...
app.post('/api/student/login', (req, res) => {
    ...
    if (username === `student_${programCode}_l${level}_1` && password === 'student123') {
        res.json({ 
            ...
            name: program?.name || programCode,
            fullname: `Student - ${program?.name || programCode}`,
            programCode, programName: program?.name || programCode, 
            level: parseInt(level)
        });
    } else {
...
app.post('/api/rooms', upload.single('syllabusFile'), (req, res) => {
    ...
    if (req.file) {
        syllabusFile = { url: `/uploads/syllabus/${req.file.filename}`, filename: req.file.originalname };
    }
    
    const room = {
        id: Date.now(),
        programCode, level: parseInt(level), courseCode, courseName: courseName || "Untitled Course",
        lecturerName: lecturerName || "Unknown Lecturer", description: description || '', objectives: objectives || '',
        syllabus: syllabusFile, materials: [], assignments: [], announcements: [],
