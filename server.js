...
function loadSuggestions() {
    try {
        if (fs.existsSync(SUGGESTIONS_FILE)) {
            globalSuggestions = JSON.parse(fs.readFileSync(SUGGESTIONS_FILE, 'utf8'));
            console.log(`✅ Loaded ${globalSuggestions.length} suggestions`);
        }
...
function saveSuggestions() {
    try {
        fs.writeFileSync(SUGGESTIONS_FILE, JSON.stringify(globalSuggestions, null, 2));
...
// Get single compartment details - FIXED
app.get('/api/compartments/detail/:compartmentId', (req, res) => {
    const compartmentId = parseInt(req.params.compartmentId);
    console.log('Looking for compartment with ID:', compartmentId);
    const compartment = courseCompartments.find(c => c.id === compartmentId);
    if (compartment) {
        console.log('Found compartment:', compartment.compartmentName);
        res.json(compartment);
    } else {
        console.log('Compartment not found with ID:', compartmentId);
...
app.delete('/api/compartments/:id', (req, res) => {
    ...
    courseCompartments = courseCompartments.filter(c => c.id !== id);
    saveCompartments();
...
app.post('/api/suggestions', (req, res) => {
    ...
    globalSuggestions.push(suggestion);
    saveSuggestions();
...
app.get('/api/suggestions/:programCode/:level', (req, res) => {
    const { programCode, level } = req.params;
    const filtered = globalSuggestions.filter(s => s.programCode === programCode && s.level === parseInt(level));
    res.json(filtered);
});
...
server.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📚 ${courseCompartments.length} course compartments`);
    console.log(`💡 ${globalSuggestions.length} suggestions`);
    console.log(`\n🔐 DEMO LOGIN:`);
