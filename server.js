...
const storage = multer.diskStorage({
    destination: (req, file, cb) => {
        ...
        if (file.fieldname === 'mediaFile') folder = 'uploads';
        cb(null, folder);
    },
...
const HOSTELS_FILE = 'hostels.json';
...
function loadMuscrecData() {
    ...
    try { if (fs.existsSync(HOSTELS_FILE)) hostels = JSON.parse(fs.readFileSync(HOSTELS_FILE)); } catch(e) {}
    try { if (fs.existsSync(BOOKINGS_FILE)) bookings = JSON.parse(fs.readFileSync(BOOKINGS_FILE)); } catch(e) {}
}
function saveMuscrecData() {
    ...
    fs.writeFileSync(HOSTELS_FILE, JSON.stringify(hostels, null, 2));
...
app.post('/api/muscrec/official/login', (req, res) => {
    ...
    if (username === 'muscrec_official' && password === 'musrec4321') {
...
app.post('/api/muscrec/posts', upload.single('mediaFile'), (req, res) => {
    ...
    let media = null;
    if (req.file) media = { type: mediaType || 'document', url: `/uploads/${req.file.filename}`, filename: req.file.originalname };
...
// ============ HOSTEL MANAGEMENT & BOOKING ============
// Get all hostels
app.get('/api/muscrec/hostels', (req, res) => res.json(hostels));
// Get single hostel by ID (for booking page)
app.get('/api/muscrec/hostels/:id', (req, res) => {
...
// Add hostel
app.post('/api/muscrec/hostels', (req, res) => {
    const { name, owner, price, distance, billsInclusive, contacts } = req.body;
    const hostel = {
        id: Date.now(),
        name: name.trim(),
        owner: owner.trim(),
        ...
        billsInclusive: billsInclusive === 'yes',
...
// Delete hostel
app.delete('/api/muscrec/hostels/:id', (req, res) => {
    const id = parseInt(req.params.id);
...
app.post('/api/muscrec/hostels/toggle/:id', (req, res) => {
    const id = parseInt(req.params.id);
    const hostel = hostels.find(h => h.id === id);
    if (!hostel) return res.status(404).json({ success: false });
    hostel.status = hostel.status === 'closed' ? 'open' : 'closed';
    saveMuscrecData();
    res.json({ success: true, status: hostel.status });
});

// Bookings
app.post('/api/muscrec/bookings', (req, res) => {
    const { hostelId, name, phone, homeAddress, arrivalDate, sex, program, regNumber, bookingRef } = req.body;
    ...
    const booking = {
        ...
        landlordContact: hostel.contacts,
        price: hostel.price,
        bookingRef: bookingRef || ('MZ' + Date.now().toString().slice(-8) + Math.floor(Math.random() * 1000)),
        name: name.trim(),
        phone: phone.trim(),
...
app.get('/api/muscrec/bookings', (req, res) => res.json(bookings));
app.delete('/api/muscrec/bookings/:id', (req, res) => {
    const id = parseInt(req.params.id);
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`📊 Main: ${ALL_PROGRAMS.length} programs | MUSCREC: ${muscrecPosts.length} posts | Hostels: ${hostels.length}`);
});
