...
function loadData() {
    try { if (fs.existsSync(ROOMS_FILE)) courseRooms = JSON.parse(fs.readFileSync(ROOMS_FILE, 'utf8')); console.log(`✅ Loaded ${courseRooms.length} course rooms`); } catch(e) { console.log('No existing data'); }
}
function saveData() { try { fs.writeFileSync(ROOMS_FILE, JSON.stringify(courseRooms, null, 2)); } catch(e) {} }
loadData();
...
wss.on('connection', (ws) => {
    ...
    ws.on('close', () => {
        if (ws.isLecturer && ws.roomId) {
            ...
            wss.clients.forEach(client => {
                if (client.roomId === ws.roomId && !client.isLecturer) {
                    client.send(JSON.stringify({ type: 'broadcast-ended' }));
                }
            });
...
app.put('/api/rooms/:roomId', upload.single('syllabusFile'), (req, res) => {
    const index = courseRooms.findIndex(r => r.id === parseInt(req.params.roomId));
    if (index !== -1) {
        if (req.file) req.body.syllabus = { url: `/uploads/syllabus/${req.file.filename}`, filename: req.file.originalname };
