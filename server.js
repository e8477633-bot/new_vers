...
// ============ ASSIGNMENT SPECIFIC ENDPOINT (FIXES ERROR) ============
app.post('/api/assignments', upload.single('assignmentFile'), (req, res) => {
    const { programCode, level, title, content, lecturerName, dueDate, points } = req.body;
...
// ============ GENERIC POSTS ROUTE (Announcements & Materials) ============
app.post('/api/posts', upload.array('materialFiles', 20), (req, res) => {
    ...
    if (media.length > 0) category = 'material';
